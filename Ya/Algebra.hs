...
instance Transformation Flat Arrow Arrow (U_II_I Constant t) (U_II_I Constant t)
	where transformation _ (U_II_I (Constant x)) = U_II_I (Constant x)

instance Covariant Semi Functor Arrow Arrow f => Transformation Flat Constant Arrow f f
	where transformation (U_I_II (Constant new)) = fo `i` \_ -> new

instance Covariant Semi Functor (Kleisli g Arrow) Arrow f
	=> Transformation Flat (Kleisli g Constant) Arrow f f
	where transformation (U_I_II (U_I_T_II (Constant new))) = fokl `i` \_ -> new

instance Contravariant Semi Functor Arrow Arrow f => Transformation Dual Constant Arrow f f
...
instance (Covariant Semi Functor Arrow Arrow f, Transformation Flat Arrow Arrow (f `T_TT_I` g) f)
...
instance Transformation Flat Arrow Arrow (Day (Flat Arrow) (/\) (/\) I I i ii) I
	where transformation (U_I_II from) = \case
		U_UU_UUU_UUUU_T_TT_I_II_III (These (These (I i) (I ii)) (U_I_II f))
			-> I (from (f (These i ii)))

instance Transformation Flat Arrow Arrow (Flat Arrow Unit) I
	where transformation (U_I_II from) (U_I_II f) = I (from (f Unit))

instance Transformation Flat Arrow Arrow (Day (Flat Arrow) (/\) (/\) (U_I_II (\/) e) (U_I_II (\/) e) i ii) (U_I_II (\/) e)
	where transformation (U_I_II from) = \case
...
instance Transformation Flat Arrow Arrow (Flat Arrow Unit) (U_I_II (\/) e)
	where transformation (U_I_II from) (U_I_II f) = U_I_II (That (from (f Unit)))

instance Transformation Flat Arrow Arrow (Day (Flat Arrow) (/\) (/\) (U_II_I (\/) e) (U_II_I (\/) e) i ii) (U_II_I (\/) e)
	where transformation (U_I_II from) = \case
...
instance Transformation Flat Arrow Arrow (Flat Arrow Unit) (U_II_I (\/) e)
