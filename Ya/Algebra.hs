...
instance
	...
	where mapping = rwr / \from -> rwr / \case
		These (These e ee) (Straight f) ->
			let These e_ e__ = _'_'_' e in
			let These ee_ ee__ = _'_'_' ee in
...
instance Mapping Straight Straight (->) (->)
	...
	where mapping = rwr / \from -> rwr `yiii` rwr `yii` rwr `yi` \(U_I_UU_II_III state) old ->
		(\(These s (Straight f)) -> from `fio` f `_'_'yi` s) (state old)
...
instance Mapping Straight Straight (->) (->)
	...
	where mapping = rwr / \from -> rwr / \case
		These (These ee eee) (Straight f) -> W_I_I_II `ha` U_I_UU_II_III `yi` \old ->
			let These new x = ee `_'_'_'` old in
			let These upd y = eee `_'_'_'` new in
