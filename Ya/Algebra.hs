...
instance
 ...
 mapping = rewrap / \from (Straight f) -> T_TT_I /
...
instance (Initial AR, Monoidal Straight Functor AR LM ML t) =>
 ...
 mapping = rewrap / \_ _ -> T_TT_I (empty @t `yo` initial @AR)
...
instance
 ...
 , Mapping Straight Straight into into (t `T_TT_I` t) t
 , Mapping Straight Straight into into (tt e `T_TT_I` t) (tt e `TT_T_I` t)
 , forall ee . Wrapper into (T_TTT_TT_I (ttt e) (tt e) t ee)
 , forall ee . Wrapper into (T_TT_I (tt e) t ee)
 , forall ee . Wrapper into (TT_T_I (tt e) t ee)
 , forall ee . Wrapper into (T_TT_I t t ee)
 , forall ee . Wrapper into (T_TT_I (T_TTT_TT_I (ttt e) (tt e) t) t ee)
 ) => Mapping Straight Straight into into
  (T_TTT_TT_I (ttt e) (tt e) t `T_TT_I` t)
  (T_TTT_TT_I (ttt e) (tt e) t) where
 mapping = rewrap / \from -> rewrap /
  map @Straight @Straight @into @into @(ttt e)
   (component @Straight @into @into @(t `T_TT_I` t) @t `compose` wrap @into
    `compose` map @Straight @Straight @into @into @t
     (wrapped (map @Straight @Straight @into @into @(tt e `T_TT_I` t) @(tt e `TT_T_I` t) from))
    )
  `compose` unwrap @into
...
instance
 ...
 , Mapping Straight Straight into into (tttt `T_TT_I` tttt) tttt
 , Mapping Straight Straight into into (tt `T_TT_I` tttt) (tt `TT_T_I` tttt)
 , Mapping Straight Straight into into (t `T_TT_I` tttt) (t `TT_T_I` tttt)
 , forall ee . Wrapper into (T_TT_I tt tttt ee)
 , forall ee . Wrapper into (TT_T_I tt tttt ee)
 , forall ee . Wrapper into (T_TT_I t tttt ee)
 , forall ee . Wrapper into (TT_T_I t tttt ee)
 , forall ee . Wrapper into (T_TT_I tttt tttt ee)
 , forall ee . Wrapper into (T_TT_I (T_TTT_TT_I ttt tt (TT_T_I t tttt)) tttt ee)
 , forall ee . Wrapper into (T_TTT_TT_I ttt tt (TT_T_I t tttt) ee)
 ) => Mapping Straight Straight into into
  (T_TTT_TT_I ttt tt (TT_T_I t tttt) `T_TT_I` tttt)
  (T_TTT_TT_I ttt tt (TT_T_I t tttt)) where
 mapping = rewrap / \from -> rewrap /
  map @Straight @Straight @into @into @ttt @ttt
   (wrap @into @(TT_T_I _ _ _)
    `compose` component @Straight @into @into @(tttt `T_TT_I` tttt) @tttt
    `compose` wrap @into @(T_TT_I tttt tttt _)
    `compose` map @Straight @Straight @into @into @tttt
     (wrapped (component @Straight @into @into @(t `T_TT_I` tttt) @(t `TT_T_I` tttt)))
    `compose` unwrap @into @(TT_T_I t tttt _)
    `compose` map @Straight @Straight @into @into @(TT_T_I t tttt) @(TT_T_I t tttt)
     (wrapped (map @Straight @Straight @into @into @(tt `T_TT_I` tttt) @(tt `TT_T_I` tttt) from))
   )
  `compose` unwrap @into
...
instance
 ...
 , Covariant Endo Semi Functor into ttt
 , Covariant Endo Semi Functor into tttt
 , Component Natural into into (t `T_TT_I` t) t
 , Component Natural into into (tt `T_TT_I` t) (tt `TT_T_I` t)
 , Component Natural into into (t `T_TT_I` tttt) (t `TT_T_I` tttt)
 , forall ee . Wrapper into (T_TT_I tt t ee)
 , forall ee . Wrapper into (TT_T_I tt t ee)
 , forall ee . Wrapper into (T_TT_I t tttt ee)
 , forall ee . Wrapper into (TT_T_I t tttt ee)
 , forall ee . Wrapper into (T_TT_I t t ee)
 , forall ee . Wrapper into (T_TT_I (T_TTT_TT_I ttt tt (TT_T_I t tttt)) t ee)
 , forall ee . Wrapper into (T_TTT_TT_I ttt tt (TT_T_I t tttt) ee)
 ) => Mapping Straight Straight into into
  (T_TTT_TT_I ttt tt (TT_T_I t tttt) `T_TT_I` t)
  (T_TTT_TT_I ttt tt (TT_T_I t tttt)) where
 mapping = rewrap / \from -> rewrap /
  map @Straight @Straight @into @into @ttt @ttt
   (wrap @into @(TT_T_I _ _ _)
    `compose` map @Straight @Straight @into @into @tttt
     (component @Straight @into @into @(t `T_TT_I` t) @t `compose` wrap @into @(T_TT_I _ _ _))
    `compose` unwrap @into @(TT_T_I t tttt _)
    `compose` map @Straight @Straight @into @into @(TT_T_I t tttt) @(TT_T_I t tttt)
     (wrapped (map @Straight @Straight @into @into @(tt `T_TT_I` t) @(tt `TT_T_I` t) from))
   )
  `compose` unwrap @into
...
instance Mapping Straight Straight AR AR (U_II_I LM e `T_TT_I` tt) (U_II_I LM e `TT_T_I` tt) =>
 Mapping Straight Straight AR AR
  ...
  (T_TTT_TT_I (U_I_II AR e) (U_II_I LM e) tt) where
 mapping = rewrap / \from (T_TT_I (U_I_II (U_I_UU_II_I x))) ->
...
instance
 ...
 mapping = rewrap / \from -> wrap @into @(T_TTT_TT_I _ _ _ _)
...
instance
 ...
 , Component Natural into into (t `T_TT_I` t) t
 , Component Natural into into (tt `T_TT_I` tt) tt
 , Component Natural into into (tt `T_TT_I` t) (tt `TT_T_I` t)
 , forall e . Wrapper into (((tt `T_TT_I` t) `T_TT_I` (tt `TT_T_I` t)) e)
 , forall e . Wrapper into (((tt `TT_T_I` t) `T_TT_I` (tt `TT_T_I` t)) e)
 , forall e . Wrapper into ((tt `T_TT_I` t) ((tt `TT_T_I` t) e))
 , forall e . Wrapper into ((t `T_TT_I` t) e)
 , forall e . Wrapper into ((tt `TT_T_I` t) e)
 , forall e . Wrapper into ((tt `T_TT_I` t) e)
 , forall e . Wrapper into ((tt `T_TT_I` tt) e)
 ) => Mapping Straight Straight into into (TT_T_I tt t `T_TT_I` TT_T_I tt t) (TT_T_I tt t) where
 mapping = rewrap / \from -> rewrap
  / component @Straight @into @into @(t `T_TT_I` t) @t `compose` wrap @into
  `compose` map @Straight @Straight @into @into @t @t
  (map @Straight @Straight @into @into @t @t
    (map @Straight @Straight @into @into @(tt `T_TT_I` tt) @tt from `compose` wrap @into)
    `compose` wrapped (component @Straight @into @into @(tt `T_TT_I` t) @(tt `TT_T_I` t))
...
-- TODO: generalize with limits
instance Covariant Monoidal Functor AR LM LM t =>
 Mapping Straight Straight AR AR (U_II_I LM e `T_TT_I` t) (U_II_I LM e `TT_T_I` t) where
 mapping = rewrap / \from -> rewrap / \case
...
instance {-# OVERLAPS #-} Component Natural AR AR (T_TT_I t tt) t =>
 ...
 mapping = rewrap / \from ->
  map @Straight @Straight @AR @AR @(T_TT_I t tt) @t from `compose` rewrap @_ @AR (fo @AR unwrap)
