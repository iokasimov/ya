...
instance
	...
	, Transformation Straight Functor (->) (->) (g `T_TT_I` g) g
	, Transformation Straight Functor (->) (->) (f `T_TT_I` g) (TT_T_I f g)
	) => Mapping Straight Straight (->) (->)
		(T_TT_I (f `T_TT_I` R_U_I_T_I (/\) f) g)
		(TT_T_I (f `T_TT_I` R_U_I_T_I (/\) f) g)
	where mapping = rw / \from -> rw / \(T_TT_I x) ->
		(wrapped (component @Straight @(->) @(->) @(f `T_TT_I` g) @(f `TT_T_I` g))
			/ x `yo` wrapped (map @Straight @Straight @(->) @(->)
				@(R_U_I_T_I (/\) f `T_TT_I` g)
				@(R_U_I_T_I (/\) f `TT_T_I` g) from)
			) `yo` wrap @(->) @(T_TT_I _ _ _)

-- TODO: I need to reduce transformations here
instance
	( Covariant Endo Semi Functor (->) f
	, Transformation Straight Functor (->) (->) (T_TT_I g g) g
	, Transformation Straight Functor (->) (->) (T_TT_I f g) (TT_T_I f g)
	) => Mapping Straight Straight (->) (->) (R_U_I_T_I (/\) f `T_TT_I` g) (R_U_I_T_I (/\) f `TT_T_I` g) 
	where mapping = rw / \from -> rw `i`
		\(R_U_I_T_I (Recursive (U_I_T_II (These x xs)))) ->
			component @Straight @(->) @(->) @(T_TT_I _ _)
			`compose` wrap @(->) @(T_TT_I _ _ _)
...
instance Mapping Straight Straight (->) (->) (T_TT_I (U_I_II (/\) e) (U_I_II (->) e)) I
	where mapping = rw / \from -> rw / \(U_I_II (These e (U_I_II f))) -> from `i` f e

instance Mapping Straight Straight (->) (->) I (T_TT_I (U_I_II (->) e) (U_I_II (/\) e))
...
instance
	...
	, Transformation Natural Functor (->) (->) (T_TT_I g g) g
	) => Mapping Straight Straight (->) (->)
	(T_TT_I
...
instance
	...
	, Mapping Straight Straight (->) (->) (t `T_TT_I` t) t
		...
		(T_TT_I (T_TT_TTT_I (Straight (->) e) t (Straight (/\) e)) t)
...
instance Mapping Straight Straight (->) (->) (U_I_II (/\) e `T_TT_I` g) (U_I_II (/\) e `TT_T_I` g) =>
	Mapping Straight Straight (->) (->)
	...
	where mapping = rw / \from (T_TT_I (U_I_II (W_I_I_II (U_I_UU_II_III x)))) -> 
		T_TT_TTT_I `compose` U_I_II / \old -> wrapped @(->)
			`i` map @Straight @Straight @(->) @(->) @(U_I_II (/\) e `T_TT_I` g) @(U_I_II (/\) e `TT_T_I` g) from
			`i'i` U_I_II (x old)

instance 
	( Component Natural (->) (->) (T_TT_I g g) g
...
instance Mapping Straight Straight (->) (->) (U_I_II (\/) e `T_TT_I` U_I_II (\/) e) (U_I_II (\/) e)
...
instance Covariant Monoidal Functor (->) (/\) (/\) f =>
	Mapping Straight Straight (->) (->) (U_I_II (\/) e `T_TT_I` f) (U_I_II (\/) e `TT_T_I` f)
	where mapping = rw / \from -> rw / \case
...
instance Covariant Monoidal Functor (->) (/\) (/\) g =>
	Mapping Straight Straight (->) (->) (U_I_II (/\) e `T_TT_I` g) (U_I_II (/\) e `TT_T_I` g)
	where mapping = rw / \from -> rw / \case
		U_I_II (These e x) -> x `yo` from `o` These e `o` U_I_II

instance Covariant Endo Semi Functor (->) g =>
	Mapping Straight Straight (->) (->) (g `T_TT_I` U_I_II (->) e) (g `TT_T_I` U_I_II (->) e)
