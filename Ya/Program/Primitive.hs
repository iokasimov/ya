...
adjust attr f s = let (These h x) = attr `yiz'yiz` s in x `i`f h
...
rep index = W_I_II_II `ha` U_I_UU_III_U_II_I `yi` \origin ->
 ...
 (idx origin `yiz` index) `yi'lm` (\x -> tbt (U_I_II / \index' -> (but `yii` idx origin `yiz` index') `rf` but x `yi` (index' `e` index)))
