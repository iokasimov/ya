...
instance Shiftable List (Optional `T'TT'I` Construction Optional) where
 shift way x = is
  ...
  slide_passed = enter @(State `T'I` Sliding List _ `JNT` Halts)
...
instance Shiftable Only (Construction List) where
 shift way x = is
  ...
  horizontally way = enter @(State `T'I` Scrolling Tree i `JNT` Halts)
  ...
  vertical_deep _ = enter @(State `T'I` Scrolling Tree i `JNT` Halts)
  ...
  vertical_up _ = enter @(State `T'I` Scrolling Tree i `JNT` Halts)
...
adjust way x = is `hu` (by None `lu` x) `la` is `ho'he` foi @_ @(AR) Some `li` router way `he'he'hv` x where
 ...
 expand_passed = enter @(State `T'I` Sliding List _ `JNT` Halts)
 ...
 shrink_passed = enter @(State `T'I` Sliding List _ `JNT` Halts)
  `yuk____` New `ha` State `hv__` Event `hv` pop @List `ha_` Scope `hv` focus
  `yok____` Try
  `yok____` New `ha` State `ha__` Event `ha` push `ho_'ha` Scope `ha` shaft `hv` by Passed

 -- [3 2 1] (4 5 6) [7 8 9] ---> [3 2 1] (4 5 6 7) [8 9]
 expand_future = enter @(State `T'I` Sliding List _ `JNT` Halts)
 ...
 -- [3 2 1] (4 5 6) [7 8 9] ---> [3 2 1] (4 5) [6 7 8 9]
 shrink_future = enter @(State `T'I` Sliding List _ `JNT` Halts)
