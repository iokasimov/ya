...
instance Mapping T'I'II T'I'II (AT) (AT) ((Construction t) `L` Alone `T` Void) Alone where
 mapping = rewrap `identity` \(T'I'TT'II'T'II'I source) ->
  T'I'TT'II'T'II'I `identity` \(Label (F'T'I'TT'I (Recursive (T'TT'I (T'II'I (These xxx x)))))) ->
   Alone `ha` this `ha` source `har` x `hjd` (\xx -> xxx `hjd_` that `ha` source `har` x `har` supertype xx)
...
instance {-# OVERLAPS #-} Component (AT) ((Maybe `T'TT'I` Construction Maybe) `L` Alone `T` (Void) `L` Alone `T` (Void)) Maybe where
 component = T'I'TT'II'T'II'I `identity` \case
  ...
  Label (Label (T'TT'I (T'I'II (That (F'T'I'TT'I (Recursive (T'TT'I (T'II'I (These xxx x))))))))) ->
   Exist `har` x `hjd` Label `ha` Label `ha` (Empty `hu_` T'TT'I (xxx `yo` F'T'I'TT'I) `has` (\x' -> x' `ryu` Enter @List))
...
instance Mapping T'I'II T'I'II (AR) (AR) ((List `P'T'I'TT'I` Twice `T'TT'I` List) `L` List `T` (Void)) List where
 mapping = rewrap `identity` \source (Label (T'TT'I'TTT'I (These x (T'TT'I (T'I'I (These sx xs)))))) ->
  sx `yokl` Forth `ha` Apply `ha` State `ha` Event `ha` push `bt'har__` x `yi__` that `hjd__` xs `yi__` Merge `ho` morph @List `yo__` source
  ...
  supertype unfoldings `yokl` Forth `ha` Apply `ha` State `ha` Event `ha` cover `bt'har__` tree `yi__` that `ho'yo` source where
   ...
   cover (T'TT'I'TTT'I (These parent siblings)) tree =
    Unit `hjd___` that `ha` sub @Tree
     `har__` supertype parent `ryu` Enter @Tree
     `har__` Alone `har` tree `hjd` supertype siblings
...
instance {-# OVERLAPS #-} Component (AT) ((Tree `P'T'I'TT'I` List `T'TT'I` (Alone `P'T'I'TT'I` Twice `T'TT'I` List `T'TT'I` Tree)) `L` Tree `T` Void) Tree where
 component = T'I'TT'II'T'II'I `identity` \(Label (T'TT'I'TTT'I (These x xxx))) -> x `hjd` (`hjd` xxx)
...
instance Mapping T'I'II T'I'II (AR) (AR) ((P'II'I i `T'TT'I` S'I'II Unit) `L` (P'II'I i `T'TT'I` S'I'II Unit) `T` Void) (S'I'II i) where
 mapping = rewrap `identity` \source (Label (T'TT'I (T'II'I (These x i)))) -> Empty `hu` Error i `has` Valid `ha` source `har` x
...
instance Mapping T'I'II T'I'II (AR) (AR)
 ((Alone `P'T'I'TT'I` Twice `T'TT'I` List `T'TT'I` Tree `P'T'I'TT'I` Tree) `L` Tree `T` (Void `P` Void)) Tree where
  mapping = rewrap `identity` \source (Label (T'TT'I'TTT'I (These (T'TT'I'TTT'I (These root siblings)) tree))) ->
   that `ha` sub @Tree
    `har__` supertype root `ryu` Enter @Tree
    `har__` Adapt @(Scrolling List) `ho` morph @(Nonempty List) `ho` Adapt @(Nonempty List) `ho` morph @List
     `har_` Alone tree `hjd` supertype siblings
    `yo___` source
...
--     `har_` intro @Tree `hc` focused `har_` sx `hjd` (forest `hjd` xs `yi` Merge @List @List `ho` morph @List) `yi` morph @List @(Twice `T'TT'I` List) @(Tree _)
  ...
  --  `har__` subtype @(AR) @(List `T'TT'I` Tree `T'I_` _) `ha` morph @List `ha` morph @(Nonempty List) `hc'st` basement
...
instance Component (AT) ((t `P'T'I'TT'I` tt) `L` t `T` (Void)) t where
 component = T'I'TT'II'T'II'I `identity` \(Label (T'TT'I'TTT'I (These x xs))) -> x `hjd` (`hjd` xs)

instance Component (AT) ((t `P'T'I'TT'I` tt) `L` tt `T` (Void)) tt where
 component = T'I'TT'II'T'II'I `identity` \(Label (T'TT'I'TTT'I (These x xs))) -> xs `hjd` (x `hjd`)
...
instance
 ...
 mapping = rewrap `identity` \source x -> T'TT'I `har__` x
  `kyo` Level @(Construction t) `ha` (\xx -> Label `ha` Along
   `ha` (is `hu` (source `ha` this `ha` top `har` xx `hjd` That Unit) `has` is `hu` (source `ha` this `ha` top `har` xx `hjd` This Unit))
   `ha` (\xxx -> this `ha` sub `har` xxx `hjd'ys` (Unit `ryu` Apply @t) `yokl` Apply `ha` Check `ha__` Error `hu` Error Unit `has` Valid)
   `har` xx)
...
instance Shiftable Alone (Maybe `T'TT'I` Construction Maybe) where
 ...
 shift way x = is `har` Empty `hu` (Empty Unit `hjd` x) `has_` is `ho'st` foi @_ @(AR) Exist `har_` _shift_ `bt'har` x where
 ...
 spot (These way predicate) x = foi Exist `ha` fetch `has_` is `ho'st` foi @_ @(AR) (Empty `hu'st` Empty) `har_` _spot_ `bt'har` x where
...
instance Shiftable List (Maybe `T'TT'I` Construction Maybe) where
 ...
 shift way x = is
  ...
  slide_future = Unit `ryu` Enter @(Halts `JNT` State `T` Sliding List i)
   ...
   `yuk____` Apply `ha` State `har___` Event `har` pop `ha___` Scope `har` field @(Shafted List i) `ho__'st` Scope `ha` index `har'st` Ahead
   `yok____` Check
  ...
  slide_passed = Unit `ryu` Enter @(Halts `JNT` State `T` Sliding List _)
   `yuk____` Apply `ha` State `har___` Event `har` pop `ha___` Scope `har` field @(Shafted List i) `ho__'st` Scope `ha` index `har'st` Aback
   `yok____` Check
...
instance Shiftable Alone (Construction List) where
 ...
 -- (L (P'T'I'TT'I (P'T'I'TT'I Alone (T'TT'I (T'TT'I Twice List) Tree)) Tree) Tree Void) (F'T'I'TT'I Along List)
  ...
  splash (These root descendants) = this `ha` top `har` descendants
   `hjd__` Alone `har` root `hjd_` subtype @(AR) `ha` morph `ha` Fresh @(Shafted List) `ha` subtype @(AR) @(List _) `ha` this `ha` sub `har` descendants
  ...
  scroll way unfolding tree =
   ...
   `yi_` (\x -> supertype @(AR) `ha` this `ha` field @(Alone `T'I` Tree i) `har` x `hjd__` this `ha` field @(Alone i) `har` unfolding `hjd_` subtype @(AR) @(T'TT'I _ _ _) `ha` this `ha` field @(Twice `T'TT'I` List `T'I_` Tree i) `har` x) `ha` that
